 ...
 * Todo list:
 * - custom part icon
 * - indicator for which direction to burn for corrections
 * - fix for eccentricity (WIP)
 * - warning if SOI is in likely trajectory?
...
namespace protractor
{
    ...
    {
        public KSP.IO.PluginConfiguration cfg = KSP.IO.PluginConfiguration.CreateForType<ProtractorModule>();
        ...
        public GameObject approach_obj = new GameObject("Line");
        public double
            throttle = 0,
            totaldv = 0,
            maxthrustaccel = 0,
        ...
        public int[] colwidths = new int[6] { 70, 63, 63, 71, 100, 71 };
        ...
        public void initialize()    //initializes lists of bodies, planets, and parameters
        {
            getbodies();
            getplanets();
            getmoons();
        ...
        public void drawGUI()
        {
            if (vessel == FlightGlobals.ActiveVessel)
            {
                ...
                {
                    if (GUI.Button(new Rect(Screen.width / 8, Screen.height - 34, 32, 32), protractoricon, iconstyle))
                    {
                        if (minimized == true)
                        {
                            minimized = false;
